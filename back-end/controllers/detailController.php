...
class DetailController
{
    ...
    public function getAllAttributesOneMovie($connection, $params)
    {
        ...
        if ($movies_arr != '500') {
            ...
            returnResponse('500', 'Internal Server Error iniiw.');
            return false;
        }
    }
    ...
    public function getMovieSchedules($connection, $params)
    {
        ...
        if ($schedule_arr != '500') {
            ...
            returnResponse('500', 'Internal Server Error aku.');
            return false;
        }
    }
    ...
    public function getMovieReviews($connection, $params)
    {
        ...
        $avg_rate = $review->getAverageRatings($connection, $params);
        if (($review_arr != '500')){
            $this->reviews = $review_arr;
            $this->avg_reviews = $avg_rate;
            return true;
        } else {
            $this->avg_reviews = 'NULL';
            return false;
        }
    }
    ...
    public function render()
    {
        ...
        if ($this->avg_reviews!= 'NULL'){
            foreach ($this->avg_reviews as $review){
            $html .=' | '. $this->movie['runtime'] . ' mins
                                    </div>
                                    <div class ="detail-released-time">
                                        Released date: '. date("F d, Y", strtotime($this->movie['release_date'])) . '
                                    </div>
                                    <div class="review">
                                    ...
                                    <div class="review">
                                    <img class="star-icon" src="../assets/star-icon.png">
                                            <p class="rating"><span class="rate">User Rating :' . $review['rating'].'</span><span class="per-rate">/10</span></p>
                                    </div>
            ...
            foreach ($this->schedules as $schedule) {
                ...
                $class_icon = "fa fa-chevron-circle-right fa-2x";
                    ...
                    $html .= '<tr class="list-schedule">
                                <th>'. date("F d, Y", strtotime($schedule['date'])) . ' </th>
                                <th class="time"> '. date("g:i A", strtotime($schedule['time'])) . '</th>
            ...
            $html .=' | '. $this->movie['runtime'] . ' mins
                                    ...
                                    <div class="review">
                                    <img class="star-icon" src="../assets/star-icon.png">
                                            <p class="rating"><span class="rate">User Rating : Not yet rated</span></p>
                                    </div>
                                    <div class="detail-summary">
                                        <p class="spacing"> '. $this->movie['overview'] . ' </p>
                                    </div>
